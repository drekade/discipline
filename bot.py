[12.05.2026 16:24] Drekade: import os, json, httpx, asyncio, random
...
1. НЕСКОЛЬКО СЪЁМОК (action: add_multiple_shoots) — несколько блоков с датами/местами.
   ...
   ВАЖНО: если в сообщении 2+ даты (числа месяца или дни недели) с разными локациями/описаниями — это ВСЕГДА add_multiple_shoots, даже если текст неструктурированный или смешан.
   Дни недели тоже считаются датами: середа=среда, п’ятниця=пятница, субота=суббота, неділя=воскресенье → конвертируй в ближайшую дату.
   Каждая съёмка: date, time, location, project, people, notes (всё остальное что не вошло).
1. ОДНА СЪЁМКА (action: add_shoot) — есть явная локация + желательно дата.
...
1. УДАЛЕНИЕ СЪЁМКИ (action: delete_shoot) — “удали съёмку X”
   data: {{shoot_date, shoot_location, shoot_time}}
1. ЗАВЕРШЕНИЕ ПРОЕКТА (action: complete_project) — “закончила/завершила проект X”
1. ИДЕЯ (action: add_idea) — “идея:”, “ідея:”
1. ДНЕВНИК (action: add_diary) — Катерина рассказывает про свой день: что делала, как встала, когда работала, как себя чувствовала, что было, как день прошёл.
...
1. ЛИЧНОЕ СОБЫТИЕ (action: add_event) — врач, ветеринар, школа, мероприятие, встреча
   data: {{title, date(YYYY-MM-DD), time, category, notes}}
1. НОВЫЙ ПРОЕКТ (action: add_project)
1. ОЧИСТКА ПОЛЯ (action: clear_field) — “отмени/убери/очисти заметку/ссылку”
   data: {{field: “notes”|“script”|“link”, entity: “shoot”|“project”}}
1. ОТВЕТ НА УТОЧНЕНИЕ (action: clarify_reply) — ТОЛЬКО если ТВОЙ ПРОШЛЫЙ reply был вопросом “какая дата?”/“во сколько?”/“где?”,
...
1. ЗАПРОС ИНФОРМАЦИИ (action: query) — Катерина спрашивает что-то по своим данным.
   ПРИМЕРЫ:
   • “какие люди снимались в этом месяце” → intent=list_people, period=month
[12.05.2026 16:24] Drekade: • “кто снимался на этой неделе” → intent=list_people, period=week
...
1. РАЗГОВОР (action: none) — только короткие междометия и прямые вопросы.
...
⚠️ ОБЯЗАТЕЛЬНО: твой ответ — это ВАЛИДНЫЙ JSON в одну строку с тремя ключами reply, action, data. Не пустой {{}}. Не markdown. НеПРИМЕР минимального ответа: {{“reply”:“Окей”,“action”:“none”,“data”:{{}}}}
...
[12.05.2026 16:24] Drekade: parsed[“reply”] = reply_text if reply_text else “Записала!”
return parsed
...
[12.05.2026 16:24] Drekade: items = [s for s in items if q in (s.get("project","") or "").lower()]
...
[12.05.2026 16:24] Drekade: continue
...
return InlineKeyboardMarkup([
[12.05.2026 16:24] Drekade: [InlineKeyboardButton(toggle_label, callback_data=f”toggle_proj_{proj_id}”)],
...
[12.05.2026 16:24] Drekade: async def cmd_today_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
...
if text_stripped in ("🏠 Сегодня", "📅 Съёмки", "🎬 Проекты", "💡 Идеи", "📓 Дневник", "📊 Итоги"):
    if text_stripped == "🏠 Сегодня":
        ...
        try:
            ...
            await msg.reply_text("\n".
[12.05.2026 16:24] Drekade: join(lines), reply_markup=reply_kbd())
        except Exception as e:
...
if uid in pending:
    ...
    if p.get("type") == "clarify_shoot":
        ...
        if any(low.startswith(w) for w in stop_words) or low in ("нет","ні","no"):
            ...
            return
        p = pending.
[12.05.2026 16:24] Drekade: pop(uid)
        field = p["field"]
        entity_type = p["type"]
        entity_id = p["id"]
...
[12.05.2026 16:24] Drekade: [InlineKeyboardButton("🎬 постановка", callback_data=f"stag_{new_id}_постановка"),
...
try:
    ...
    if action == "query":
        ...
        period = data.
[12.05.2026 16:24] Drekade: get("period","month")
...
if cb == "shoots":
    ...
    if upcoming:
        buttons.
[12.05.2026 16:24] Drekade: append([InlineKeyboardButton("— предстоящие —", callback_data="noop")])
        for s in upcoming[:5]:
            icon = "🔜"
...
if cb.startswith("addlink_proj_"):
    proj_id = int(cb.split("_")[2])
[12.05.2026 16:24] Drekade: pending[uid] = {"type":"project","id":proj_id,"field":"link"}
...
[12.05.2026 16:24] Drekade: ds = len([s for s in shoots if s.get("status")=="снято" and s.get("created_at","")>week_ago])
